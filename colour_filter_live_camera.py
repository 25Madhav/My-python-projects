...
def apply_filter(image, ftype):
    ...
    if ftype == "red_tint":
        ...
        img_abs = cv2.convertScaleAbs(lap)
        img = cv2.cvtColor(img_abs, cv2.COLOR_GRAY2BGR)
    return img
...
def main():
    ...
    if not cap.isOpened():
        ...
        return
    ftype = "original"
    print("Keys: r=Red, g=Green, b=Blue, s=Sobel, c=Canny, t=Cartoon, a=Gaussian, l=Laplacian, q=Quit")
    while True:
        ret, frame = cap.read()
        if not ret:
